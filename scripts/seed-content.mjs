...
// Phase 3: placeholder per-room GLBs (procedural, generated by
// scripts/generate-placeholder-rooms.mjs). Real tenant uploads via the
// media library replace these with zero code change.
const ROOM_GLB = {
  living: "/models/rooms/living-room.glb",
  kitchen: "/models/rooms/kitchen.glb",
  bedroom: "/models/rooms/bedroom.glb",
  study: "/models/rooms/study.glb",
};

const ROOM_DESCRIPTIONS = {
  living:
    "The living room anchors the floor plan - a single bay for long evenings. The joinery is drawn in-house, the stone laid to the same axis as the window light.",
  kitchen:
    "The kitchen is the workshop of the house: a counter run specified stone-on-wood, upper cabinets at sightline, and an island the family actually uses at breakfast.",
  bedroom:
    "A room for quiet. The bed wall stays clear of sockets and switches, the wardrobe runs floor-to-ceiling, and the light turns indirect after dark.",
  study:
    "The study doubles as the second desk of the house - a bookshelf run, a pendant over the working surface, and a chair that doesn't sacrifice the afternoon.",
};

const PROJECT_ROOMS = [
  { slug: "casa-mira", rooms: ["living", "kitchen", "bedroom"] },
  { slug: "nalanda-house", rooms: ["living", "study", "bedroom"] },
  { slug: "salt-flats", rooms: ["kitchen", "living", "study"] },
];
...
async function seedPostgres() {
  ...
  // Phase 3: seed placeholder rooms for the room-by-room walkthrough.
  // Only projects with zero rooms get seeded (tenant rooms are never
  // clobbered); --force wipes and reseeds.
  if (FORCE) {
    await pool.query(`DELETE FROM project_rooms`);
  }
  for (const pr of PROJECT_ROOMS) {
    const proj = (
      await pool.query(`SELECT id FROM projects WHERE slug = $1 LIMIT 1`, [
        pr.slug,
      ])
    ).rows[0];
    if (!proj) continue;
    const existing = (
      await pool.query(
        `SELECT COUNT(*)::int AS n FROM project_rooms WHERE project_id = $1`,
        [proj.id]
      )
    ).rows[0].n;
    if (existing > 0) continue;
    let order = 0;
    for (const key of pr.rooms) {
      await pool.query(
        `INSERT INTO project_rooms
           (project_id, name, slug, description, model_3d,
            cover_media_id, hotspots, order_index, is_published)
         VALUES ($1,$2,$3,$4,$5,NULL,NULL,$6,true)`,
        [
          proj.id,
          ROOM_TITLES[key],
          key,
          ROOM_DESCRIPTIONS[key],
          ROOM_GLB[key],
          order++,
        ]
      );
...
async function seedSqlite() {
  ...
  // Phase 3: placeholder rooms for the room-by-room walkthrough
  // (mirrors seedPostgres; tenant rooms never clobbered).
  if (FORCE) {
    try {
      prep(`DELETE FROM project_rooms`).run();
    } catch {}
  }
  const roomInsert = prep(
    `INSERT INTO project_rooms
       (project_id, name, slug, description, model_3d,
        cover_media_id, hotspots, order_index, is_published)
     VALUES (?, ?, ?, ?, ?, NULL, NULL, ?, 1)`
  ...
  for (const pr of PROJECT_ROOMS) {
    const proj = prep(`SELECT id FROM projects WHERE slug = ? LIMIT 1`).get(
      pr.slug
    );
    if (!proj) continue;
    const existing = prep(
      `SELECT COUNT(*) AS n FROM project_rooms WHERE project_id = ?`
    ).get(proj.id);
