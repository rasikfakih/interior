...
export async function GET(
  ...
  const row = await pgOne(`SELECT * FROM team_members WHERE id = $1`, [
    Number(id),
  ]);
...
export async function PUT(
  ...
  try {
    ...
    if (typeof d.bio === "string") { updates.push(`bio = $${i++}`); args.push(d.bio.slice(0, 2000)); }
    if (typeof d.photo === "string") { updates.push(`photo = $${i++}`); args.push(d.photo.slice(0, 500)); }
    if (typeof d.order === "number") { updates.push(`"order" = $${i++}`); args.push(d.order); }
    ...
    const q = await pgQuery(
      `UPDATE team_members SET ${updates.join(", ")} WHERE id = $${i} RETURNING *`,
      args
    );
    const row = q.rows?.[0];
    if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
...
export async function DELETE(
  ...
  const { id } = await params;
  await ensureMigrated();
  await pgQuery(`DELETE FROM team_members WHERE id = $1`, [Number(id)]);
  bump({ kind: "team" });
