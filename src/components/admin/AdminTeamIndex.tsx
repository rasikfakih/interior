...
export default function AdminTeamIndex() {
  ...
  async function publishToggle(r: Row) {
    setBusyId(r.id);
    try {
      const rr = await fetch(`/api/team/${r.id}`, {
        method: "PUT",
  ...
  async function destroy(r: Row) {
    if (!confirm(`Delete team member "${r.name}"? This removes the row only.`)) {
      return;
    }
    setBusyId(r.id);
    try {
  ...
  const filtered = useMemo(() => {
    ...
    const sortKey: Record<Sort, (r: Row) => string | number> = {
      name: (r) => (r.name ?? "").toLowerCase(),
      role: (r) => (r.role ?? "").toLowerCase(),
      order: (r) => r.order ?? 9999,
    };
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <h2 className="text-3xl md:text-4xl tracking-tighter">Team</h2>
      ...
      {loading ? (
        ...
        <p className="text-ink-mute">
          No team members match. Create one using the button above.
        </p>
      ) : (
