...
### 2026-08-15 - Full system audit (TS-ID-038 follow-up)

Full SaaS audit executed and documented in docs/AUDIT.md. Static: tsc 0, vitest 3/3, eslint 0 errors (48 warnings). Gates: boolean-sql 0 violations, themes 8/8, verify-deploy OK, lint-changed clean, contrast 84/0 (needs CONTRAST_ADMIN_EMAIL/PASSWORD since admin creds rotated). Smoke: modules 2-10 run against a disposable postgres:17 container on :5544 with next start on :3010 (existing ethinterior.vercel.app-bound build, license matched, operator license untouched) - 407 assertions, 0 failures (52/37/45/38/45/55/52/50/33). Live site: public 200, unknown portal/proposal tokens 404, anon APIs 401, admin login session role admin, authed APIs 200, client-host middleware 404 vs apex 200. Real Supabase verified untouched (production rows: 1 lead, 1 project, 1 proposal, 1 board, 1 BOQ, 1 log, 1 snag, 2 AI generations). Fixed during audit: superadmin credential hash (was a third unknown string; re-hashed with delivered password, live-verified role superadmin). Note: resetForSmokes() comment saying "the operator's DB has none" is now outdated - production has real module data; the destructive reset must only ever target disposable databases. Gaps: no automated browser-flow suite in CI (smokes are API-level), realtime board channel untested by assertions, real AI provider unverified, Lighthouse score not persisted.

### 2026-08-15 - Unsplash image swap (post-audit polish)

Swapped every remaining /demo/*.jpg reference in src to a curated Unsplash catalog (src/lib/images.ts), all 36 photo IDs HTTP-verified (200) before use. Covers: homepage hero, kanban column empty-state art (KANBAN_ART), materials grid + empty state, board materials sidebar, board canvas (dead fallback const removed), diary + snags empty states (site/process photos), BOQ + boards empty states, client portal board thumbnail, public proposal boards fallback. Material fallbacks are category-aware via materialImageUrl(). Demo project fallback lists extended 4 -> 6 rooms (added bathroom, outdoor deck). next.config already allowed images.unsplash.com; dropped unoptimized flags so next/image optimizes. public/demo/*.jpg kept on disk for storage-path compatibility and verify-deploy. Live DB projects already used Unsplash URLs (3 published rows) - untouched. Verified: tsc 0, eslint 0 errors, boolean-sql 0 violations, verify-deploy OK, vitest 3/3, full build green, preview renders hero + horizontal projects all via /_next/image Unsplash (0 local demo). Committed 216cdfe, pushed; CI + Vercel integration will roll it out.

### 2026-08-15 - Verified Unsplash swap on production

CI + smoke both green on a01bae2; Vercel deployed a01bae2 READY. Production https://ethinterior.vercel.app homepage serves 3 unique Unsplash photos through /_next/image (hero 1600585154526 + the 3 DB projects), 0 /demo/ references, 200 OK. All 21 catalog photo IDs in src/lib/images.ts re-verified HTTP 200. Image-swap commit 216cdfe had been auto-deployed by the integration before a01bae2 (docs-only) replaced it.

### 2026-08-15 - M0: unify v1 and v2 design systems (Studio OS + demo agency architecture)

Direction confirmed with owner: Studio OS is the SaaS product on ethinterior.vercel.app, Etihad Interiors is the demo agency tenant (future /demo route group), and the two share one design grammar. M0 executed (commit c0b4ed3): (1) one display voice - Instrument Serif is now --font-display everywhere (globals @theme + theme.ts customizerVars now always emits --font-display, defaulting to a new 'instrument' font token; Newsreader remains --font-serif for editorial body; admin theme picker now offers instrument first). (2) .dark token block completed with the full v2 set (paper/paper-soft/clay/moss/amber/line-soft/shadow-soft) so v2 surfaces flip in dark mode. (3) HomeV2 + HorizontalProjects hardcoded hexes swept to semantic tokens (text-ink, text-ink-mute, text-accent-deep, bg-canvas, bg-[var(--paper-soft)]); fixed brand moments stay pinned (amber CTAs get dark:text guard, ink band, scrim). (4) Avatars + navbar language control moved rounded-full -> 8px radius. Notable: a stale dev server (pid 11132) was silently owning :3000 with a pre-edit CSS cache and defeated the font change - killed all strays and restarted clean. Verified: tsc 0, build green, contrast walker 84/0, boolean-sql 0, eslint 0; preview confirms homepage + /about in one voice and dark mode flipping cleanly. Next: M1 route split (/ = Studio OS SaaS, /demo = Etihad Interiors agency frame + Powered by Studio OS hook).

### 2026-08-15 - M1: root split into Studio OS SaaS and /demo agency

Root is now the Studio OS SaaS marketing site (SaasNav with Features/Pricing/Live demo/Sign in/Get started, SaasHome with product hero + stats + features + live-tenant selected work + pricing + CTA, SaasFooter). Etihad Interiors moved to /demo with its own agency frame: (demo)/demo/layout.tsx (Powered by Studio OS strip -> /, Navbar with agency links + homeHref=/demo, Footer with Studio OS credit, SmoothScroll/Grain/Cursor motion), studio-first home page (Homes drawn, built, and lived in + HorizontalProjects + system teaser + contact CTA), and six sections relocated via git mv (work, about, contact, journal, voices, themes). Old URLs 301 (permanent) to /demo/*. Fixed "FEATURES.map is not a function": server components cannot consume exports from "use client" modules, so SaasHome is now a client component. Windows quirk: git mv failed with Permission denied while dev server watchers held dirs; killing strays (07:14-07:27 batch) freed them. Walker route list updated; walker now needs a fresh prod build (dev clobbers .next) - ran after rebuild, 80 pass / 0 fail. Build green with all 9 /demo routes registered; redirects 308 to /demo/*; eslint 0. Commit 871ecdb pushed. Next: M2 SaaS depth pages (pricing, changelog, quickstart) + verify M0/M1 on production after CI.
